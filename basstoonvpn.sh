...
REMOTE_SSH_ADDR=$1
MODE=$2
if [[ (-z "$REMOTE_SSH_ADDR") || ("$MODE" != "add" && "$MODE" != "reset") ]]
then
	echo "Usage: $0 root@<remote server address> <subcommand: add/reset> [mode arguments...]" 1>&2
	exit 1
fi
shift 2 # from here onward the arguments $1, $2... just refer to the subcommand arguments
...
then
	DISPLAY_MODE=$1
	if [[ "$DISPLAY_MODE" != "text" && "$DISPLAY_MODE" != "qr" ]]
	then
		echo "Usage: $PROGNAME root@<remote server address> add <display mode: text/qr>" 1>&2
		exit 1
	fi
	if  [[ "$DISPLAY_MODE" == "qr" ]]
	then
		...
		then
			echo "Error: qrencode command not found. Please install the qrencode package." 1>&2
			exit 1
		fi
		read -p "The QR code for WireGuard interfaces is large. Zoom out your terminal, then press Enter to continue." 1>&2
	fi
	...
	then
		echo "$CLIENT_CONFIG"
	elif  [[ "$DISPLAY_MODE" == "qr" ]]
	then
		echo "$CLIENT_CONFIG" | qrencode -t ansi
	else
		echo "Unsupported display mode: $DISPLAY_MODE" 1>&2
		exit 1
	fi
